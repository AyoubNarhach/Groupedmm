...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			modernee_create_observer( 'modernee-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				modernee_gutenberg_editor_init();
			} );
		} else {
		 	modernee_gutenberg_editor_init();
		}
	...
	function modernee_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'modernee_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'modernee_inited' ) ) {
	...
	// Init on page load
	function modernee_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = modernee_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + MODERNEE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + MODERNEE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in MODERNEE_STORAGE['override_classes'] ) {
				$obj.addClass( MODERNEE_STORAGE['override_classes'][i].replace( '%s', MODERNEE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( modernee_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && MODERNEE_STORAGE['sidebar_position'] != 'hide' ) {
				if ( modernee_apply_filters( 'modernee_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'modernee_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( modernee_apply_filters( 'modernee_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				modernee_remove_observer( 'modernee-check-editor-styles-wrapper' );
				modernee_create_observer( 'modernee-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'modernee_inited' );
	}
} );
