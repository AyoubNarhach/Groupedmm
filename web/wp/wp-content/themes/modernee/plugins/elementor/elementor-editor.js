...
jQuery( document ).ready(
	function() {
		...
		modernee_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="modernee_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.modernee_post_editor' );
						if ( link.length > 0 ) {
							modernee_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.modernee_post_editor', function(e) {
						modernee_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('modernee_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function modernee_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
