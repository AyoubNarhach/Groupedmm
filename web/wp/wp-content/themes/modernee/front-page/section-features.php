<div class="front_page_section front_page_section_features<?php
	$modernee_scheme = modernee_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $modernee_scheme ) && ! modernee_is_inherit( $modernee_scheme ) ) {
		echo ' scheme_' . esc_attr( $modernee_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( modernee_get_theme_option( 'front_page_features_paddings' ) );
	if ( modernee_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$modernee_css      = '';
		$modernee_bg_image = modernee_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$modernee_anchor_icon = modernee_get_theme_option( 'front_page_features_anchor_icon' );
	$modernee_anchor_text = modernee_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( modernee_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$modernee_css      = '';
			$modernee_bg_mask  = modernee_get_theme_option( 'front_page_features_bg_mask' );
			$modernee_bg_color_type = modernee_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $modernee_bg_color_type ) {
				$modernee_bg_color = modernee_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$modernee_caption = modernee_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $modernee_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $modernee_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $modernee_caption, 'modernee_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$modernee_description = modernee_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $modernee_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $modernee_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $modernee_description ), 'modernee_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! modernee_exists_trx_addons() ) {
						...
						modernee_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
