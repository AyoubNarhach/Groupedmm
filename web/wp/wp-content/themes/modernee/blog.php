...
if ( function_exists( 'modernee_elementor_is_preview' ) && modernee_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			modernee_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	modernee_new_main_query(
		array(
			'post_type'      => modernee_get_theme_option( 'post_type' ),
			'category'       => modernee_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'modernee_filter_get_template_part', modernee_blog_archive_get_template() ) );
}
